...
plt.ion()
STATIC = False; START = 230
# STATIC = True; START = 0; STATIC_OBS_POS = (0.5, 0.1)  # 115 start and (0.3, 0.1) obs pos is default

# CBF param
CBF_GAMMA = 4  # Represents how close the agent can come to the obstacle (higher = closer)
...
FREQ = 500  # Control frequency of the universal robot E series
TIME = 1  # Desired time to complete the task?

########################################################################################################################
DELTA_TIME = 1/FREQ  # Time period
N_STEPS = int(TIME/(1/FREQ))  # Total number of steps for the simulation
...
def distance_derivative():
    ...
    hx = sym.lambdify([x_ee, x_obs, R, Rw], expr=alt_hx)
...
def get_force(r, o):
    ...
    curr_dist = dist_f(r, o, 0, 0)
    if curr_dist < APF_Q:
        nabla_d = dist_deriv(r, o, 0, 0).flatten()
...
def use_cbf(r, desired_pos, o):
    ...
    G = -dist_deriv(r, o, 0, 0)  # CBF derivative
    h = CBF_GAMMA * dist_f(r, o, 0, 0)  # CBF exponential gamma*hx

    vel = solve_ls(R, s, G, h, lb=np.array([MIN_VEL, MIN_VEL]), ub=np.array([MAX_VEL, MAX_VEL]), solver="osqp")  # osqp or cvxopt
    return vel
...
obstacle_traj = traj.ctraj(SE3(0.5, 0.5, 0), SE3(0.5, -0.5, 0), N_STEPS).t
...
for i in range(1, N_STEPS-1):
    ...
    if STATIC:
        curr_obstacle_pos = np.array(STATIC_OBS_POS); next_obstacle_pos = np.array(STATIC_OBS_POS)
    else:
        curr_obstacle_pos = obstacle_traj[i-1, :2]; next_obstacle_pos = obstacle_traj[i, :2]
...
vel_ax[0][0].plot(range(0, 500), np.round(robot_vel_history[:, 0], 3), label="x desired velocity", color='red'); vel_ax[0][0].legend()
vel_ax[0][1].plot(range(0, 500), np.round(robot_vel_history[:, 1], 3), label="y desired velocity", color='red'); vel_ax[0][1].legend()
vel_ax[1][0].plot(range(0, 500), np.round(robot_vel_apf_history[:, 0], 3), label="x apf velocity", color='blue'); vel_ax[1][0].legend()
vel_ax[1][1].plot(range(0, 500), np.round(robot_vel_apf_history[:, 1], 3), label="y apf velocity", color='blue'); vel_ax[1][1].legend()
vel_ax[2][0].plot(range(0, 500), np.round(robot_vel_cbf_history[:, 0], 3), label="x cbf velocity", color='green'); vel_ax[2][0].legend()
vel_ax[2][1].plot(range(0, 500), np.round(robot_vel_cbf_history[:, 1], 3), label="y cbf velocity", color='green'); vel_ax[2][1].legend()

# Plot position error
pos_fig.suptitle('Position error over time')
pos_ax[0][0].plot(range(0, 500), np.round(robot_traj[:, 0] - robot_vel_apf_history[:, 0], 3), label="x position error apf", color='red'); pos_ax[0][0].legend()
pos_ax[0][1].plot(range(0, 500), np.round(robot_traj[:, 1] - robot_vel_apf_history[:, 1], 3), label="y position error apf", color='red'); pos_ax[0][1].legend()
pos_ax[1][0].plot(range(0, 500), np.round(robot_traj[:, 0] - robot_vel_cbf_history[:, 0], 3), label="x position error cbf", color='blue'); pos_ax[1][0].legend()
pos_ax[1][1].plot(range(0, 500), np.round(robot_traj[:, 1] - robot_vel_cbf_history[:, 1], 3), label="y position error cbf", color='blue'); pos_ax[1][1].legend()

# Initial plot to set labels for the axes
fig.suptitle('Position over time and repulsive forces')
ax[0].scatter(robot_traj[0, 0], robot_traj[0, 1], marker='.', color='blue', label='desired')
ax[0].scatter(robot_pos_apf_history[0, 0], robot_pos_apf_history[0, 1], marker='.', color='cyan', label='apf')
ax[0].scatter(robot_pos_cbf_history[0, 0], robot_pos_cbf_history[0, 1], marker='.', color='black', label='cbf')
ax[1].scatter(0, apf_force_history[0, 0], marker='_', color='red', label="x repulsion")
ax[1].scatter(0, apf_force_history[0, 1], marker='_', color='blue', label="y repulsion")

for i in range(1, N_STEPS):
    # Plots trajectories
    ax[0].scatter(robot_traj[i, 0], robot_traj[i, 1], marker='.', color='blue')
    ax[0].scatter(robot_pos_apf_history[i, 0], robot_pos_apf_history[i, 1], marker='.', color='cyan')
    ax[0].scatter(robot_pos_cbf_history[i, 0], robot_pos_cbf_history[i, 1], marker='.', color='black')
    if STATIC:
        ax[0].scatter(STATIC_OBS_POS[0], STATIC_OBS_POS[1], marker='*', color='red')
    else:
        ax[0].scatter(obstacle_traj[i, 0], obstacle_traj[i, 1], marker='*', color='red')
